"""
Data Augmentation for Model M2 (Green Channel + Normalization + Histogram Eq)
=============================================================================
Augmentation Strategy:
  - Horizontal flip, Vertical flip
  - Random rotation (±25°)
  - Random crop (85-95% of image, resized back)
  - NO Gaussian blur (images already histogram-equalized, blur would degrade)
...
GAUSSIAN_BLUR_KERNEL = 0  # Disabled — already equalized
RANDOM_SEED = 42
...
def generate_augmentations(img, rng, count):
    ...
    transforms = [
        ...
        lambda im: cv2.flip(random_crop(im, rng), 0),       # v-flip + crop
        lambda im: random_rotation(random_crop(im, rng), rng),  # crop + rotation
        lambda im: cv2.flip(random_rotation(im, rng), -1),  # 180° + rotation
    ]
...
def main():
    ...
    print(f"Model M2 — Data Augmentation")
    print(f"{'=' * 50}")
