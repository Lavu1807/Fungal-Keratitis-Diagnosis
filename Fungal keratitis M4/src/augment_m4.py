"""
Data Augmentation for Model M4 (Spatial Resize + Complex Pipeline)
==================================================================
Augmentation Strategy:
  - Horizontal flip, Vertical flip
  - Random rotation (±30°)
  - Random crop (85-95% of image, resized back to 256×256)
  - Light Gaussian blur (kernel=3)
  - Extra augmentations on minority class (normal) to balance with FK
  
Note: M4 images are 256×256 (resized during preprocessing).
...
def main():
    ...
    print(f"Model M4 — Data Augmentation")
    print(f"{'=' * 50}")
    print(f"Original counts: FK={n_fk}, Normal={n_normal}")
    print(f"Params: rotation={ROTATION_RANGE}, crop={CROP_FACTOR_RANGE}, "
          f"blur_kernel={GAUSSIAN_BLUR_KERNEL}")
    print(f"Note: Images are 256x256 (resized during preprocessing)")
